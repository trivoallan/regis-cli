...
The MR description contains a link to the **Review App** where the report will be hosted.

=== 3. Report Inspection
Once the analysis job completes, the report is:
*   **Committed** back to the branch (if `GITLAB_TOKEN` is configured).
*   **Deployed** to a GitLab Pages Review App at `https://<namespace>.gitlab.io/<project>/review-<mr-id>/`.

=== 4. Governance
Security teams can review the MR, check the compliance score in the Review App, and decide whether to merge the analysis request into the main registry branch.
